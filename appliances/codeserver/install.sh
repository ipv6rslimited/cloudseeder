...
echo
echo
(cd $HOME/.ipv6rs/appliances/codeserver/ && podman build -t codeserver . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/codeserver:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo The password is at /home/codeserver/.config/code-server/config.yaml
echo
echo
