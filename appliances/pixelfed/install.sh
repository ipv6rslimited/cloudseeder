...
echo
echo
(cd $HOME/.ipv6rs/appliances/pixelfed/ && podman build -t pixelfed . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  --env ADMINUSERNAME="__ADMINUSERNAME" \
  --env ADMINPASSWORD="__ADMINPASSWORD" \
  --env ADMINNAME="__ADMINNAME" \
  --env EMAIL="__EMAIL" \
  --name __NAME \
  --security-opt "label=disable" \
  localhost/pixelfed:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
