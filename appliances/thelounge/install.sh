...
echo
echo
(cd $HOME/.ipv6rs/appliances/thelounge/ && podman build -t thelounge . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/thelounge:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo
echo Make an account at by going to /root/ and running ./make_account.sh
