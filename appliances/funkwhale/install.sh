...
echo
echo
(cd $HOME/.ipv6rs/appliances/funkwhale/ && podman build -t funkwhale . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/funkwhale:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo Make your admin account by typing ./make_admin.sh in the /root folder
echo
echo
