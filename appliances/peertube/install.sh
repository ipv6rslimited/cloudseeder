...
echo
echo
(cd $HOME/.ipv6rs/appliances/peertube/ && podman build -t peertube . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/peertube:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo Password for root on peertube is located in /root/.peertube_password
echo You can type cat /root/.peertube_password to see it
echo
echo
