...
echo
echo
(cd $HOME/.ipv6rs/appliances/miniflux/ && podman build -t miniflux . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/miniflux:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo
echo Run ./mkadmin.sh in the root folder and then
