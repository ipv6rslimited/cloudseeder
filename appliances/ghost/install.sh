...
echo
echo
(cd $HOME/.ipv6rs/appliances/ghost/ && podman build -t ghost . && podman run -d \
  --cap-add=NET_ADMIN \
  ...
  localhost/ghost:latest && podman exec __NAME bash -c /.root.sh)
echo
echo
echo
echo
...
echo
echo Finish setup at: https://__SERVERNAME/ghost
echo
echo
