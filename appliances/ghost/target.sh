...
su - casper -c "PATH=/usr/bin:$PATH cd /var/www/$SERVERNAME && ghost install --db=mysql --dbhost=localhost --dbname=ghost --dbuser=root --dbpass=$DBPASSWORD --url=https://$SERVERNAME --process=systemd --no-prompt --sslemail=$EMAIL 5.82.9"
